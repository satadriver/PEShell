...
string Section::insertSection(int flag, const char *secname, const char srcfiles[16][256], int srcfilescnt,char * outname) {
	int ret = 0;
...
// 	}
	...
	char * lpdate = 0;
	int filesize = 0;
	ret = FileHelper::fileReader(srcfilename.c_str(), &lpdate, &filesize);
	if (ret <= 0)
	{
	...
	PIMAGE_DOS_HEADER dos = (PIMAGE_DOS_HEADER)lpdate;
	...
	DWORD checksum = PEParser::checksumPE((unsigned char*)lpdate, filesize);
	...
	int lastmemalignsize = secalign - (lastsec->Misc.VirtualSize & (secalign -1)) + lastsec->Misc.VirtualSize;
	...
	ret = FileHelper::fileWriter(newfilename.c_str(), lpdate, filesize,TRUE);
